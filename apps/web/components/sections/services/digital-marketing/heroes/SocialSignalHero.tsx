...
export function SocialSignalHero({ className }: { className?: string }) {
	...
	return (
		<section className={cn('relative overflow-hidden py-16 lg:py-24', className)}>
			...
			<div className='absolute left-1/2 top-1/2 -z-10 size-[30rem] -translate-x-1/2 -translate-y-1/2 rounded-full border border-pastel-lilac-border/45 bg-pastel-lilac/20' />
			...
			<motion.div variants={containerAnimation} initial='hidden' animate='visible' className='section-container'>
				<div className='mx-auto max-w-5xl text-center'>
					...
					<motion.h1
						variants={itemAnimation}
						className='mx-auto mt-6 max-w-4xl font-display text-4xl font-bold tracking-tight text-foreground sm:text-5xl lg:text-6xl'>
					...
					<motion.p variants={itemAnimation} className='mx-auto mt-5 max-w-3xl text-base leading-relaxed text-muted-foreground sm:text-lg'>
						Social growth systems inspired by your reference direction: consistent messaging, platform-native content, and measurable conversion pathways.
					</motion.p>
				...
				<motion.div variants={itemAnimation} className='mx-auto mt-10 max-w-5xl rounded-3xl border border-pastel-sky-border bg-pastel-sky/40 p-4 shadow-sm'>
					<div className='overflow-hidden rounded-2xl border border-border/50 bg-background/85 p-4'>
						<div className='mb-4 flex items-center justify-between border-b border-border/60 pb-3'>
							<div className='text-xs font-semibold uppercase tracking-[0.14em] text-pastel-sky-ink'>Signal Dashboard</div>
							<div className='text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground'>Live Narrative Feed</div>
						</div>
						<div className='grid gap-3 md:grid-cols-12'>
							...
							<div className='md:col-span-9'>
								<div className='relative h-44 rounded-xl border border-border/60 bg-background/80 p-4'>
									<div className='absolute right-4 top-4 rounded-xl border border-pastel-mint-border bg-pastel-mint/75 px-3 py-2 text-left'>
										<div className='text-[10px] font-semibold uppercase tracking-wide text-pastel-mint-ink'>Signal Strength</div>
						...
						{SIGNAL_ITEMS.map((item, index) => (
							<div
								...
								className={cn(
									...
									index === 0
										? 'border-pastel-lilac-border bg-pastel-lilac/55'
										: index === 1
											? 'border-pastel-mint-border bg-pastel-mint/55'
								...
								<div className='inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground'>
									{index === 0 ? <TrendingUp className='size-3.5' aria-hidden /> : index === 1 ? <Megaphone className='size-3.5' aria-hidden /> : <Radio className='size-3.5' aria-hidden />}
									{item.label}
								</div>
